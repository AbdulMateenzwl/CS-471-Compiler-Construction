...
class SymbolTable {
   ...
   public:
    void insert(const string &name, int value) {
        if (!exists(name)) {
            cout << "Error: Variable " << name << " already in symbol table." << endl;
            exit(1);
        }
        table[name] = value;
    }
...
class Parser {
   ...
   private:
    ...
    void parseDeclaration() {
        ...
        expect(T_ID);
        expect(T_SEMICOLON);
        symbolTable.insert(var_name, 0);
    }

    void parseDeclarationWithAssignment() {
        expect(T_INT);
        string var_name = tokens[pos].value;
        expect(T_ID);
        expect(T_ASSIGN);
        int val = stoi(tokens[pos].value);
        expect(T_NUM);
        expect(T_SEMICOLON);
        symbolTable.insert(var_name, val);
    }
